<script lang="ts">
    ...
    const numeroDigitado = (num: string) => {
        calc = calc.numeroDigitado(num);
    };
    const operacaoDigitada = (op: string) => {
        calc = calc.operacaoDigitada(op);
    };
    const pontoDigitado = () => {
        calc = calc.pontoDigitado();
    };
    const limpar = () => {
        calc = calc.limparTela();
    };
    const calcular = () => {
        calc = calc.calcular();
    };
